...
void PortM_Init(void){
	SYSCTL_RCGCGPIO_R |= SYSCTL_RCGCGPIO_R11;					// activate clock for Port P
	while((SYSCTL_PRGPIO_R&SYSCTL_PRGPIO_R11) == 0){};	// allow time for clock to stabilize
  GPIO_PORTM_DIR_R &= ~0x02;    										// make PM1 in 
  GPIO_PORTM_DEN_R |= 0x02;     										// enable digital I/O on PM1
	
	GPIO_PORTM_PCTL_R &= ~0x000000F0;	 								//  configure PM1 as GPIO 
	GPIO_PORTM_AMSEL_R &= ~0x02;											//  disable analog functionality on PM1		
	GPIO_PORTM_PUR_R |= 0x02;													//	enable weak pull up resistor
}
...
void PortM_Interrupt_Init(void){
		...
		GPIO_PORTM_IS_R = 0;     						// (Step 1) PP1 is edge-sensitive 
		GPIO_PORTM_IBE_R = 0;    						//     			PP1 is not both edges 
		GPIO_PORTM_IEV_R = 0;    						//     			PP1 falling edge event 
		GPIO_PORTM_ICR_R = 0x02;      			// 					clear interrupt flag by setting proper bit in ICR register
		GPIO_PORTM_IM_R = 0x02;      				// 					arm interrupt on PJ1 by setting proper bit in IM register
    
		NVIC_EN2_R = 0x00000100;           // (Step 2) enable interrupt 77 in NVIC
	
		NVIC_PRI18_R = 0x00080000; 					// (Step 4) set interrupt priority 4
